...
class Cooperative(models.Model):
    ...
    logo = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="Caminho relativo ao arquivo dentro de static/",
    )
    ...
    def save(self, *args, **kwargs):
        if Cooperative.objects.exists() and not self.pk:
            raise ValueError("Já existe uma cooperativa cadastrada. Só pode haver uma.")
        super().save(*args, **kwargs)

    @classmethod
    def update_or_create(cls, name, logo_path=None):
        cooperative, created = cls.objects.update_or_create(
...
class UserProfile(AbstractUser):
    ...
    institution = models.ForeignKey(
        Institution, on_delete=models.PROTECT, null=True, verbose_name="Instituição"
    )
...
class Call(models.Model):
    ...
    institution = models.ForeignKey(
        Institution, on_delete=models.PROTECT, verbose_name="Instituição"
    )
    ...
    def clean(self):
        if self.active:
            active_calls = Call.objects.filter(
                institution=self.institution, active=True
            )
            if self.pk:
...
class Product(models.Model):
    ...
    unit = models.CharField(
        max_length=3, choices=CHOICES, verbose_name="Unidade de Medida"
    )
...
class CallProduct(models.Model):
    call = models.ForeignKey(
        Call, on_delete=models.CASCADE, related_name="product", verbose_name="Chamada"
    )
    product = models.ForeignKey(
        Product, on_delete=models.PROTECT, related_name="call", verbose_name="Produto"
    )
...
class Order(models.Model):
    ...
    user = models.ForeignKey(
        UserProfile, on_delete=models.PROTECT, verbose_name="Autor"
    )
    institution = models.ForeignKey(
        Institution, on_delete=models.PROTECT, verbose_name="Instituição"
    )
    ...
    timestamp = models.DateTimeField(
        auto_now=False, auto_now_add=True, verbose_name="Timestamp"
    )
    status = models.CharField(
        max_length=10, choices=CHOICES, default="pending", verbose_name="Situação"
    )
    ...
    def available_products(self):
        if (self.status == "approved") or (self.status == "delivered"):
            products = OrderedProduct.objects.filter(
                order=self, status__in=("parcial", "available")
            )
            return products
...
class OrderedProduct(models.Model):
    ...
    order = models.ForeignKey(
        ...
        related_name="call_products",
        verbose_name="Pedido",
    )
    call_product = models.ForeignKey(
        CallProduct,
        on_delete=models.PROTECT,
        related_name="order",
        verbose_name="Produto da Chamada",
    )
    ordered_quantity = models.DecimalField(
        max_digits=7, decimal_places=2, verbose_name="Quant. Pedida"
    )
    available_quantity = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name="Quant. Disponível",
    )
    status = models.CharField(
        max_length=10, choices=CHOICES, default="available", verbose_name="Situação"
    )
